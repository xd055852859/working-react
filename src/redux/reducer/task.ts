...
export const task = (state = defaultState, action : any) => {
  switch (action.type) {
      ...
      if (cardIndex !== -1) {
        ...
        if (action.data.groupArray.length > 0) {
          action
            .data
            .groupArray
            .unshift(action.data.groupArray.splice(cardIndex, 1)[0]);
          action
            .data
            .cardArray
            .unshift(action.data.cardArray.splice(cardIndex, 1)[0]);
        }
      }
    ...
    case actionTypes.EDIT_TASK_SUCCESS:
      ...
      } else if ((headerIndex === 1 || headerIndex === 2) && state.workingTaskArray) {
        state.workingTaskArray = state
          .workingTaskArray
          .map((taskItem : any, taskIndex : number) => {
    ...
    case actionTypes.GET_CALENDAR_LIST_SUCCESS:
      ...
      action
        .data
        .forEach((item : any) => {
          if (item.taskEndDate >= moment().startOf('day').valueOf() && item.taskEndDate <= moment().endOf('day').valueOf() && item.taskEndDate >= moment().valueOf()) {
