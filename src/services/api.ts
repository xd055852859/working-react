...
const auth = {
  ...
  getMainGroupKey() {
    return request.patch(HOME_URL + '/group/createMainGroup', {token: auth_token});
  },
  ...
  dealCareFriendOrGroup(type : number, friendOrGroupKey : string, status : number) {
  ...
  getWorkingConfigInfo() {
    return request.post(HOME_URL + '/account/getWorkingConfigInfo', {token: auth_token});
  },
  ...
  getGroupLog(groupKey : string, startTime : number, endTime : number, curPage : number, perPage : number) {
  ...
  getUserLog(targetUKey : string, startTime : number, endTime : number, curPage : number, perPage : number) {
  ...
  monthEnergyWeb(startTime : number, endTime : number, type : number, targetUGKey : string) {
  ...
  getClockInCommentList(clockInKey : number | string, curPage : number, perPage : number) {
...
const task = {
  ...
  getTaskList(typeBoard1 : number, targetUGKey : string, finishPercentArray : string, fileDay?: number, endTime?: number, isAddTodayFinish?: number) {
  ...
  addTask(groupKey : number | string, groupRole : number | string, labelKey : any, executorKey?: any, title?: string, parentCardKey?: string, cardIndex?: number, type?: number, taskType?: number, finishPercent?: number, taskEndDate?: number) {
  ...
  getGroupDataTask(groupKey : string, finishPercentArray : any, startTime : number | null, endTime : number) {
    return request.post(HOME_URL + '/card/getTeamCareTask', {
      token: auth_token,
      groupKey: groupKey,
      finishPercentArray: finishPercentArray,
...
const group = {
  getGroup(listType : number, simple?: number | null, sortType?: number, groupKey?: string) {
  ...
  //设置默认执行者
  setLabelOrGroupExecutorKey(labelOrGroupKey : string | number, executorKey : string | number, type : number) {
  ...
  //获取创群模板
  getTemplateTypeList() {
    return request.post(HOME_URL + '/group/getTemplateTypeList', {token: auth_token});
  },
  getTemplateListAccordingType(curPage : number, type
    ?
    : any) {
